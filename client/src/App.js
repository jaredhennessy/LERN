...
function App() {
  ...
  useEffect(() => {
    const checkLoggedIn = async () => {
      ...
      const tokenResponse = await Axios.post("/api/users/checkToken", null, { headers: { "authorization": tokenLocal } });
    ...
    <Router>
      <UserContext.Provider value={{ userData, setUserData }}>
        <div className="App">
          ...
          {userData.user ? <a href="/" onClick={logout}>Log Out</a> :
            (
              <>
          ...
          <Switch>
            <Route exact path="/"><h1>Homepage</h1></Route>
