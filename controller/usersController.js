...
// Set the course's completeDate for the specified user when called
function completeCourse(userCourseId) {
  const newDate = Date.now();
  User.findOneAndUpdate(
    { "courses._id": userCourseId },
...
module.exports = {
  ...
  // Return courses where course owner matches username
  findUserCourses: function (req, res) {
    User.findOne({
      _id: req.params.id
    })
      .select(
        "username email image courses dateCreated enrolled coursesEnrolled coursesInProgress coursesCompleted percentComplete coursesTaught"
      )
      .populate([
        {
          path: "courses.Course",
  ...
  // Set the course's startDate for the specified user when called
  ...
  // Set the course's currentPage for the specified user when called
  // Increment or decrement according to whether "next" or "prev" is specified
  // When moving "next" from the last page in a course, call the completeCourse function above
  movePage: function (req, res) {
    ...
    User.aggregate(query).exec((err, data) => {
      if (err) {
        ...
        if (
          dir === "next" &&
          data[0].courses.currentPage === req.body.endPage
        ) {
          dir = "complete";
